...
public class ContactService {
    ...
    public List<ContactDto> importContacts(Long userId, MultipartFile file) throws Exception {
        ...
        if (filename != null && filename.endsWith(".csv")) {
            contacts = importFromCsv(file, user);
        } else if (filename != null && filename.endsWith(".xlsx")) {
            contacts = importFromExcel(file, user);
        } else {
    ...
    private List<Contact> importFromCsv(MultipartFile file, User user) throws Exception {
        ...
        try (BufferedReader fileReader = new BufferedReader(new InputStreamReader(file.getInputStream(), StandardCharsets.UTF_8));
            ...
            for (CSVRecord csvRecord : csvRecords) {
                Contact contact = createContactFromCsvRecord(csvRecord, user);
    ...
    private Contact createContactFromCsvRecord(CSVRecord csvRecord, User user) {
        Contact contact = new Contact();
    ...
    private List<Contact> importFromExcel(MultipartFile file, User user) throws Exception {
        List<Contact> contacts = new ArrayList<>();
        try (Workbook workbook = new XSSFWorkbook(file.getInputStream())) {
            ...
            for (int i = 1; i <= sheet.getLastRowNum(); i++) {
                ...
                Contact contact = createContactFromExcelRow(row, firstNameIdx, lastNameIdx, emailIdx, phoneIdx, titleIdx, user);
                contacts.add(contact);
            }
        }
        return contacts;
    ...
    private Contact createContactFromExcelRow(Row row, int firstNameIdx, int lastNameIdx, int emailIdx, int phoneIdx, int titleIdx, User user) {
        Contact contact = new Contact();
    ...
    public byte[] exportContactsToCsv(Long userId) throws Exception {
        ...
        try (PrintWriter pw = new PrintWriter(new OutputStreamWriter(out, StandardCharsets.UTF_8));
            ...
            for (Contact contact : contacts) {
                writeContactToCsv(contact, csvPrinter);
            }
        }
        return out.toByteArray();
    }

    private void writeContactToCsv(Contact contact, org.apache.commons.csv.CSVPrinter csvPrinter) throws Exception {
    ...
    public byte[] exportContactsToExcel(Long userId) throws Exception {
        ...
        try (Workbook workbook = new XSSFWorkbook()) {
            ...
            for (Contact contact : contacts) {
                ...
                writeContactToExcelRow(contact, row);
            }
    ...
    private void writeContactToExcelRow(Contact contact, Row row) {
